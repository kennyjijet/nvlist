...
public final class EngineVersion {
    ...
    public static void checkVersion(String engineMinVersion, String engineTargetVersion)
        ...
        if (StringUtil.compareVersion(VERSION_STRING, engineMinVersion) < 0) {
            ...
            throw new UnsupportedVersionException(String.format("Engine version number (%s) "
                    + "is below the minimum acceptable version (%s)", VERSION_STRING, engineMinVersion));
        }

        if (StringUtil.compareVersion(engineTargetVersion, MIN_COMPAT_VERSION) < 0) {
