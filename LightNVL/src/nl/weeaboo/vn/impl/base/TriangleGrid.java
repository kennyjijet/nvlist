...
import static nl.weeaboo.vn.impl.base.TriangleGrid.TextureWrap.REPEAT_BOTH;
import static nl.weeaboo.vn.impl.base.TriangleGrid.TextureWrap.REPEAT_X;
import static nl.weeaboo.vn.impl.base.TriangleGrid.TextureWrap.REPEAT_Y;
...
public final class TriangleGrid implements Serializable {
	...
	private TriangleGrid(int rows, int cols, FloatBuffer pos, FloatBuffer[] tex) {
		this.rows = rows;
		this.cols = cols;
		this.pos = BufferUtil.toArray(pos);
	...
	private static TriangleGrid layout(Area2D[] bounds, Area2D[] texBounds, TextureWrap[] wrap) {
		...
		pos.rewind();
		for (FloatBuffer buf : texs) {
			buf.rewind();
		}
		return new TriangleGrid(rows, cols, pos, texs);
	}
			...
			Area2D bounds, Area2D texBounds, TextureWrap wrap)
	{
		...
		double u;
		if (wrap != REPEAT_X && wrap != REPEAT_BOTH) {
		...
		double v;
		if (wrap != REPEAT_Y && wrap != REPEAT_BOTH) {
