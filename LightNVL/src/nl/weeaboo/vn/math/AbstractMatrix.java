...
abstract class AbstractMatrix implements Serializable {
	...
	public boolean equals(AbstractMatrix m, double epsilon) {
		...
		if (epsilon == 0.0) {
			...
			return Math.abs(m00 - m.m00) <= epsilon
				&& Math.abs(m01 - m.m01) <= epsilon
				&& Math.abs(m02 - m.m02) <= epsilon
				&& Math.abs(m10 - m.m10) <= epsilon
				&& Math.abs(m11 - m.m11) <= epsilon
				&& Math.abs(m12 - m.m12) <= epsilon;
		}
	}
